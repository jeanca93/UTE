...
public class TipoAulaModel {
	...
	@NotifyChange({"allTipoAulaStatus", "displayEdit"})
    ...
    public void nuevoTipoAula() {
		...
		Window window = (Window)Executions.createComponents(
                "/WEB-INF/include/Mantenimiento/TipoAula/vtnTipoAula.zul", null, null);
        window.doModal();
    }
	...
	public void eliminarTipoAula(){
	    ...
	    for(TipoAulaStatus tipoaulaStatus:allTipoAulaStatus){
	    	if(tipoaulaStatus.isSeleccionado())
	...
	@Command
	@NotifyChange("allTipoAulaStatus")
	public void selectedCheckBox(@BindingParam("CheckEvent") CheckEvent evt){
		for(TipoAulaStatus tipoaualStatus:allTipoAulaStatus){
			if(!tipoaualStatus.isEditingStatus())
				tipoaualStatus.setSeleccionado(evt.isChecked());
		}
	}
	...
	public ListModelList<TipoAulaStatus> getAllTipoAulaStatus(){
		return allTipoAulaStatus;
	}
