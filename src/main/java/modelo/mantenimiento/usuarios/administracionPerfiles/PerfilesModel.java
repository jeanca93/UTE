...
public class PerfilesModel {
	...
	@NotifyChange({"allPerfilStatus", "displayEdit"})
	...
	public void eliminarPerfiles(){
		...
		for(PerfilStatus perfilStatus:allPerfilStatus){
			if(perfilStatus.isSeleccionado())
	...
	public void asignarPermisos(){
		...
		if(perfiles.size() > 1){
			...
			Window window = (Window)Executions.createComponents(
					"/WEB-INF/include/Mantenimiento/Usuarios/AdministracionPerfiles/vtnPermisosPerfil.zul", null, parameters);
	...
	public ListModelList<PerfilStatus> getAllPerfilStatus() {
		return allPerfilStatus;
	}
